...
export class MeetingModelAdapter extends AbstractModel implements ForInteractingWithMeetingModel {
  async save(meeting: MeetingEntity, booker: UserEntity): Promise<MeetingEntity | Error> {
    if (!this.connection) {
    ...
    const prepare = await this.connection.prepare(`
      ...
      values (?, ?, ?)
    `)
    const [result] = await prepare.execute([meeting.start, meeting.event.id, booker.id]);
